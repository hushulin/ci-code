...
$resuilt= file_get_contents("http://copy.formaxmarket.com/Forex/quotes?syms=GBPUSD&_=".time(),false,$context);
  $resuilt=json_decode($resuilt);
  $res=object_array($resuilt);
  $formatted = sprintf("%.5f", rand(-20,20)/100000);
  $lastprice=$res['GBPUSD']['bid']+$formatted ;
  if($lastprice>0.5){
mysql_query("INSERT INTO st_type (type, price,time) 
VALUES ('GBPUSD', ".sprintf("%.5f",$lastprice).", ".time().")");  //英兑美
echo date("Y-m-d H:i:s",time())."-".$res['GBPUSD']['bid']."\n";
