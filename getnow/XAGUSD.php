...
$resuilt= file_get_contents("http://copy.formaxmarket.com/Forex/quotes?syms=XAGUSD&_=".time(),false,$context);
  $resuilt=json_decode($resuilt);
  $res=object_array($resuilt);
  $formatted = sprintf("%.3f", rand(-20,20)/1000);
  $lastprice=$res['XAGUSD']['bid']+$formatted ;
  if($lastprice>0.5){
mysql_query("INSERT INTO st_type (type, price,time) 
VALUES ('XAGUSD', ".sprintf("%.5f",$lastprice).", ".time().")");  //白银
echo date("Y-m-d H:i:s",time())."-".$res['XAGUSD']['bid']."\n";
